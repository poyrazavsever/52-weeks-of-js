...
1. **Minimal Estetik** — Her bileşende `rounded-sm`, `shadow-none`, `border`
2. **Erişilebilirlik** — Radix UI primitifleri ile WCAG uyumlu klavye navigasyonu ve ARIA
3. **Varyant Odaklı** — `cva` ile tip-güvenli varyant yönetimi
4. **Kompozisyon** — Küçük, birleştirilebilir parçalar (yapılandırma değil, bileşim)
...
- **Font:** `Inter` (sans-serif birincil), `Agbalumo` (ikincil/cursive)
- **Birincil Renk:** `--color-primary: #dc2626` (red-600)
- **Base Stiller:** Başlıklar için `font-bold tracking-tight`
- **Utility:** `font-secondary` — ikincil font kullanımı için
...
Brutalist kırmızı offset gölgeli logo bileşeni.
